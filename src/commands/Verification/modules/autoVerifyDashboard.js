...
function buildDashboardEmbed(cfg, guild, conflictSummary = '') {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '🏷️ Zielrolle', value: autoVerifyRole ? autoVerifyRole.toString() : '`Nicht gesetzt`', inline: true },
            { name: '🎯 Kriterium', value: criteriaDescription, inline: true },
    ...
    if (conflictSummary) {
        embed.addFields({ name: '⚠️ Einrichtungskonflikte', value: conflictSummary, inline: false });
    }
    ...
    return new StringSelectMenuBuilder()
        ...
        .addOptions(
            ...
            new StringSelectMenuOptionBuilder()
                ...
                .setDescription('Mindestalter des Kontos in Tagen festlegen')
...
async function refreshDashboard(rootInteraction, cfg, guildId, client) {
    try {
        ...
        try {
            ...
            const conflicts = [
                verificationEnabled ? 'Das normale Verifizierungssystem ist aktiviert' : null,
            ...
            logger.warn('Konflikte für Auto-Verify-Dashboard konnten nicht abgerufen werden:', error.message);
        }
        ...
        logger.debug('Auto-Verify-Dashboard konnte nicht aktualisiert werden (Interaktion eventuell abgelaufen):', error.message);
    }
}
    ...
    async execute(interaction, config, client) {
        try {
            ...
            if (!guildConfig.verification?.autoVerify?.enabled) {
                ...
                if (verificationEnabled) blockingMessage.push('Das normale Verifizierungssystem ist aktiviert');
            ...
            collector.on('collect', async selectInteraction => {
                ...
                try {
                    ...
                    if (error instanceof TitanBotError) {
                        logger.debug(`AutoVerify-Konfigurationsvalidierungsfehler: ${error.message}`);
                    } else {
            ...
            btnCollector.on('collect', async btnInteraction => {
                try {
                    ...
                    logger.debug('Button-Interaktionsfehler:', err.message);
                }
            });
            ...
            collector.on('end', async (collected, reason) => {
                if (reason === 'time') {
                    ...
                    try {
                        ...
                        logger.debug('Dashboard konnte nach Zeitablauf nicht aktualisiert werden:', error.message);
                    }
                }
            });
        } catch (error) {
...
async function handleCriteria(selectInteraction, rootInteraction, guildConfig, guildId, client) {
    ...
    criteriaCollector.on('collect', async criteriaInteraction => {
        ...
        await criteriaInteraction.followUp({
            embeds: [successEmbed('✅ Kriterium aktualisiert', `Das Auto-Verifizierungskriterium wurde auf **${criteriaDisplay}** geändert.`)],
            flags: MessageFlags.Ephemeral,
        });
        ...
        if (role.id === rootInteraction.guild.id || role.managed) {
            await roleInteraction.followUp({
                embeds: [
                    errorEmbed(
                        'Ungültige Rolle',
                        'Bitte wähle eine normale, zuweisbare Rolle (nicht @everyone oder eine Bot-verwaltete Rolle).',
                    ),
                ],
                flags: MessageFlags.Ephemeral,
            });
        ...
        if (role.position >= botMember.roles.highest.position) {
            await roleInteraction.followUp({
                embeds: [
                    errorEmbed(
                        'Rolle zu hoch',
                        'Die ausgewählte Rolle muss in der Rollenhierarchie des Servers unter meiner höchsten Rolle liegen.',
                    ),
                ],
                flags: MessageFlags.Ephemeral,
            });
        ...
        .addComponents(
            new ActionRowBuilder().addComponents(
                new TextInputBuilder()
                    ...
                    .setLabel('Mindestalter des Kontos (Tage)')
